...
#load(file = here('4 Generated Datasets', 'PrePost_mediation_models_difference_score_May30.RData'))
#load(file = here('4 Generated Datasets', 'PrePost_mediation_models_residualized.RData'))
...
estimates <-
  ...
  map_df(c('estimate')) %>%
  t() %>%
  magrittr::set_colnames(c('a_estimate','b_estimate','cprime_estimate','ab_estimate')) %>%
...
standard_errors <-
  ...
  map_df(c('std.error')) %>%
  t() %>%
  magrittr::set_colnames(c('a_stderr','b_stderr','cprime_stderr','ab_stderr')) %>%
  as_tibble(rownames = 'model') %>%
  mutate(across(is.numeric, finalfit::round_tidy,2))
...
cis <-
  ...
  map_df(c('ci.95')) %>%
  t() %>%
  magrittr::set_colnames(c('a_ci.95','b_ci.95','cprime_ci.95','ab_ci.95')) %>%
  as_tibble(rownames = 'model') %>%
  mutate(across(is.numeric, finalfit::round_tidy,2))
...
p_values <-
  ...
  map_df(c('p.value')) %>%
  t()%>%
  magrittr::set_colnames(c('a_pval','b_pval','cprime_pval','ab_pval')) %>%
  as_tibble(rownames = 'model') %>%
