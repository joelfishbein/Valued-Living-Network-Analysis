...
write_rds(x = PreToMid.fit, file = here('Fitted model objects','PreToMid.fit'))
...
write_rds(x = MidToPost.fit, file = here('Fitted model objects','MidToPost.fit'))
...
write_rds(x = PostTo3MFU.fit, file = here('Fitted model objects','PostTo3MFU.fit'))
...
write_rds(x = my3mTo6mFU.fit , file = here('Fitted model objects','my3mTo6mFU.fit'))
...
write_rds(x = PreToPost.fit , file = here('Fitted model objects','PreToPost.fit'))

```

\newpage

# Error in resample -- see below

Here is the first few rows of the n x p matrix

```{r}

head(getDataForInterval(ChangeScores_data, 'PreToPost'))

```

Here is the call to run the network, and then resample. Resample returns an error.

```{r message=TRUE, warning=TRUE}

PreToPost.fit <- runChangeScoreNetwork(ChangeScores_data, 'PreToPost')

my_data <- getDataForInterval(ChangeScores_data, 'PreToPost')

PreToPost.fit.boot <- resample(object = PreToPost.fit, 
                               data = my_data, 
                               nB = 50)
...
write_rds(x = PreTo6mFU.fit , file = here('Fitted model objects','PreTo6mFU.fit'))
