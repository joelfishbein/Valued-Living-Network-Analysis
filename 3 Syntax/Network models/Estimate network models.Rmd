...
```{r}
PreToMid.fit <- runBootstrappedChangeScoreNetwork(ChangeScores_data, 'PreToMid')

write_rds(x = PreToMid.fit, file = here('Fitted model objects','PreToMid.fit.boot'))
...
```{r}
MidToPost.fit <- runBootstrappedChangeScoreNetwork(ChangeScores_data, 'MidToPost')
...
```{r}
PostTo3MFU.fit <- runBootstrappedChangeScoreNetwork(ChangeScores_data, 'PostTo3mFU')
...
```{r}
my3mTo6mFU.fit <- runBootstrappedChangeScoreNetwork(ChangeScores_data, '3mFUTo6mFU')
...
PreToPost.fit <- runBootstrappedChangeScoreNetwork(ChangeScores_data, 'PreToPost')

write_rds(x = PreToPost.fit , file = here('Fitted model objects','PreToPost.fit.boot'))
